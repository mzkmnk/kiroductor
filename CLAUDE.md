...
## コミットメッセージ・ドキュメントの命名規則

コミットメッセージおよびドキュメント（`docs/` 配下）には、**アプリ名（`kiroductor`）を記載しない**こと。

- **良い例**: `feat(renderer): PromptInput フォームを実装する`
- **悪い例**: `feat(kiroductor): PromptInput フォームを実装する`

コミットメッセージはスコープに `renderer` / `main` / `preload` / `e2e` などの**レイヤー名**を使う。
...
`src/renderer/components/ui/` および `src/renderer/lib/` 以下のファイルは `pnpm dlx shadcn@latest add` で自動生成されるファイルである。**手動で変更しないこと**。
カスタマイズは呼び出し側コンポーネントの `className` プロパティで行う（`cn()` ユーティリティを使用）。

これらのディレクトリは ESLint・Prettier の対象から除外している。

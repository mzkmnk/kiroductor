...
- [x] リクエスト・レスポンスの型（`ReadTextFileRequest` / `ReadTextFileResponse`）を定義する
  - 型は `@agentclientprotocol/sdk` から `import type` する（独自定義は不要）
- [x] `ReadTextFileMethod` クラスを作成し、ファイルシステム（`fs`）を依存注入で受け取るようにする
- [x] 指定されたパスのファイルを UTF-8 で読み込む処理を実装する
- [x] 読み込んだ内容を `{ content }` の形で返すよう実装する
- [x] テスト: 存在するファイルを指定すると内容が返ること
  - **AC**: `readFile` モックが指定パス・`"utf-8"` で呼ばれ、`{ content: "file content" }` が返る
- [x] テスト: 存在しないファイルを指定するとエラーが投げられること
  - **AC**: `readFile` モックが `ENOENT` エラーを返すと、同じエラーが `throw` される
- [x] 備考: `params.limit`（最大行数）/ `params.line`（開始行）は MVP では無視する（[`implementation-plan.md`](../design/implementation-plan.md) 参照、将来対応は [Phase 8](./phase8.md)）
  - 動作確認で kiro-cli が `limit: 3` を送信することを確認済み。MVP では全文返却で問題ない

## writeTextFile — エージェントが指定した内容をファイルへ書き込む

- [x] リクエスト・レスポンスの型（`WriteTextFileRequest` / `WriteTextFileResponse`）を定義する
  - 型は `@agentclientprotocol/sdk` から `import type` する（独自定義は不要）
- [x] `WriteTextFileMethod` クラスを作成し、ファイルシステム（`fs`）を依存注入で受け取るようにする
- [x] 指定されたパスへ内容を UTF-8 で書き込む処理を実装する
- [x] 書き込み成功時に空オブジェクト `{}` を返すよう実装する
- [x] テスト: ファイルが正しく書き込まれること
  - **AC**: `writeFile` モックが `(path, content, "utf-8")` で呼ばれ、`{}` が返る
- [x] テスト: 書き込みに失敗した場合エラーが投げられること
  - **AC**: `writeFile` モックがエラーを返すと、同じエラーが `throw` される

## requestPermission — エージェントが操作の許可を求めてきたときに応答する

- [x] リクエスト・レスポンスの型（`RequestPermissionRequest` / `RequestPermissionResponse`）を定義する
  - 型は `@agentclientprotocol/sdk` から `import type` する（独自定義は不要）
- [x] `RequestPermissionMethod` クラスを作成し、通知サービスを依存注入で受け取るようにする
- [x] MVP として最初の選択肢を自動承認する処理を実装する（`optionId` を返す）
  - `params.options[0].optionId` を `outcome.optionId` として返す
  - `outcome.outcome` は `"selected"` 固定
- [x] 承認内容をレンダラー（画面）へ通知する処理を実装する
- [x] テスト: 最初のオプション ID が返されること
  - **AC**: `options[0].optionId` が `outcome.optionId` に入り、`outcome.outcome === "selected"` で返る
- [x] テスト: レンダラーへの通知が実行されること
...
- [x] `SessionUpdateMethod` クラスを作成し、メッセージ Repository と通知サービスを依存注入で受け取るようにする

- [x] `agent_message_chunk` イベント: エージェント返答のテキストを追記し、画面へ通知する処理を実装する
...
- [x] `tool_call` イベント: ツール呼び出しの開始を記録し、画面へ通知する処理を実装する
...
- [x] `tool_call_update` イベント: ツール呼び出しの結果を更新し、画面へ通知する処理を実装する
  - **AC**: `updateToolCall(toolCallId, { status, result: JSON.stringify(rawOutput) })` が呼ばれる（status は SDK の `ToolCallStatus` をそのまま渡す）
  - **AC**: `rawOutput` が `undefined` の場合、`result` は更新しない
  - **AC**: `notificationService.sendToRenderer` が呼ばれる

- [x] 上記以外のイベント: 画面へ転送するだけのフォールスルー処理を実装する
...
- [x] `KiroductorClientHandler implements Client` のクラスを作成する
- [x] 各メソッドインスタンスをコンストラクタで受け取るよう実装する
- [x] 受け取ったリクエストを対応するメソッドへ委譲するだけの実装をする
- [x] `extNotification` を実装し、`_kiro.dev/` 拡張通知を無視する

...
const streaming = this.messageRepo
  .getAll()
...
### 5. `ToolCallStatus` の扱い

`MessageRepository.ToolCallMessage.status` には SDK の `ToolCallStatus` をそのまま使う（マッピング不要）。
...
`SessionUpdateMethod` は SDK から受け取った status をそのまま Repository に渡す。
